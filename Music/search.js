...
async execute(message,args,client) {
    ...
    if (!args.length)
      return attentionembed(message,`<a:ehdgsisagreatplace:915408203309809684> ${message.client.prefix}${module.exports.name} <Video Name>`)
    ...
    if (!message.member.voice.channel)
      return attentionembed(message,"<a:sugeditidhw8hd72duheufduh3duhe:919238876608987188>First, go to The Voice Channel.")
       //If not in the same channel return error
    if (serverQueue && channel !== message.guild.me.voice.channel)
    return attentionembed(message, `<:eieh38su:915200640773410828>Come on, my voice channel`);
    //define search
    ...
    try {
      ...
      {max: 2, time: 80000, errors: ['time'],} ).then(collected => {
      ...
      if(response === "error"){
        //send error message
        attentionembed(message,"<a:cycycydtd6xyc6d6r6r6r6d6emoji_53:918260430470774855>Select emojis !");
